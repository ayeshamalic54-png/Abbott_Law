...
export default function Settings() {
  ...
  // Logo Settings
  const [llbLogoUrl, setLlbLogoUrl] = useState("");
  ...
  // Load saved settings into state when they're fetched
  useEffect(() => {
    if (savedSettings && Object.keys(savedSettings).length > 0) {
      if (savedSettings.collegeName) setCollegeName(savedSettings.collegeName);
      if (savedSettings.collegeAddress) setCollegeAddress(savedSettings.collegeAddress);
      if (savedSettings.collegePhone) setCollegePhone(savedSettings.collegePhone);
      if (savedSettings.collegeEmail) setCollegeEmail(savedSettings.collegeEmail);
      if (savedSettings.collegeWebsite) setCollegeWebsite(savedSettings.collegeWebsite);
      if (savedSettings.currentSession) setCurrentSession(savedSettings.currentSession);
      if (savedSettings.currentSemester) setCurrentSemester(savedSettings.currentSemester);
      if (savedSettings.admissionOpen) setAdmissionOpen(savedSettings.admissionOpen === 'true');
      if (savedSettings.lateFeeEnabled) setLateFeeEnabled(savedSettings.lateFeeEnabled === 'true');
      if (savedSettings.lateFeeAmount) setLateFeeAmount(savedSettings.lateFeeAmount);
      if (savedSettings.gracePeriod) setGracePeriod(savedSettings.gracePeriod);
      if (savedSettings.smsEnabled) setSmsEnabled(savedSettings.smsEnabled === 'true');
      if (savedSettings.emailEnabled) setEmailEnabled(savedSettings.emailEnabled === 'true');
      if (savedSettings.autoBackupEnabled) setAutoBackupEnabled(savedSettings.autoBackupEnabled === 'true');
      if (savedSettings.backupTime) setBackupTime(savedSettings.backupTime);
      if (savedSettings.backupFrequency) setBackupFrequency(savedSettings.backupFrequency);
      if (savedSettings.retentionDays) setRetentionDays(savedSettings.retentionDays);
      if (savedSettings.llbLogoUrl) setLlbLogoUrl(savedSettings.llbLogoUrl);
      if (savedSettings.groupLogoUrl) setGroupLogoUrl(savedSettings.groupLogoUrl);
    }
  }, [savedSettings]);
  ...
  const saveSettingsMutation = useMutation({
    ...
    onSuccess: (_, variables) => {
      ...
      const section = variables._section || 'Settings';
      toast({
        title: "Settings Saved",
        description: `${section} settings have been saved to the database`,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const handleSaveSettings = (section: string) => {
    let settingsToSave: Record<string, string> = { _section: section };
    
    switch (section) {
      case "College Information":
        settingsToSave = {
          ...settingsToSave,
          collegeName,
          collegeAddress,
      ...
      case "Academic":
        settingsToSave = {
          ...
          currentSession,
          currentSemester,
          admissionOpen: admissionOpen.toString(),
        };
        break;
      case "Fee":
        settingsToSave = {
          ...settingsToSave,
          lateFeeEnabled: lateFeeEnabled.toString(),
          lateFeeAmount,
          gracePeriod,
        };
        break;
  ...
  const handleLogoUpload = (event: React.ChangeEvent<HTMLInputElement>, type: 'llb' | 'group') => {
    const file = event.target.files?.[0];
    if (!file) return;
    ...
    reader.onload = (e) => {
      ...
      if (type === 'llb') {
        setLlbLogoUrl(dataUrl);
      } else {
        setGroupLogoUrl(dataUrl);
      }
      toast({
        title: "Logo Uploaded",
        description: "Click 'Save Appearance Settings' to persist the changes",
      });
    };
        ...
        <TabsContent value="appearance" className="space-y-6">
          <Card>
            ...
            <CardContent className="space-y-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className="border-2 border-dashed rounded-lg p-6 text-center">
                    ...
                    <input
                      ...
                      className="hidden"
                      id="llb-logo-input"
                      onChange={(e) => handleLogoUpload(e, 'llb')}
                      data-testid="input-llb-logo"
                    />
                    <Button 
                      variant="outline" 
                      size="sm" 
                      onClick={() => document.getElementById('llb-logo-input')?.click()}
                      data-testid="button-upload-llb-logo"
                    >
                      <Upload className="h-4 w-4 mr-2" />
                      {llbLogoUrl ? 'Change Logo' : 'Upload Logo'}
                    </Button>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="border-2 border-dashed rounded-lg p-6 text-center">
                    ...
                    <input
                      ...
                      className="hidden"
                      id="group-logo-input"
                      onChange={(e) => handleLogoUpload(e, 'group')}
                      data-testid="input-group-logo"
                    />
                    <Button 
                      variant="outline" 
                      size="sm" 
                      onClick={() => document.getElementById('group-logo-input')?.click()}
                      data-testid="button-upload-group-logo"
                    >
                      <Upload className="h-4 w-4 mr-2" />
                      {groupLogoUrl ? 'Change Logo' : 'Upload Logo'}
                    </Button>
                  </div>
                </div>
              </div>
              ...
              >
                <Save className="h-4 w-4 mr-2" />
                {saveSettingsMutation.isPending ? 'Saving...' : 'Save Appearance Settings'}
              </Button>
            </CardContent>
          </Card>
        </TabsContent>
