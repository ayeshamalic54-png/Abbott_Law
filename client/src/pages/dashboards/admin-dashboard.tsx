...
import { GraduationCap, Users, DollarSign, AlertCircle, TrendingUp, Calendar, FileText, BookOpen, Wallet, TrendingDown, ArrowUpCircle } from "lucide-react";
...
export default function AdminDashboard() {
  ...
  // Calculate today's total income (LLB + Group)
  const todayTotalIncome = (stats?.todayLlbIncome || 0) + (stats?.todayGroupIncome || 0);
  ...
  const statCards = [
    ...
    {
      title: "Daily Income",
      value: `Rs ${todayTotalIncome.toLocaleString()}`,
      icon: ArrowUpCircle,
    ...
    {
      title: "Daily Expense",
      value: `Rs ${(stats?.todayExpenses || 0).toLocaleString()}`,
      icon: TrendingDown,
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900">
      <div className="container mx-auto p-3 sm:p-4 md:p-6 space-y-4 sm:space-y-6">
        ...
        <div className="grid gap-6 md:grid-cols-2">
          {/* Report 1: Combined Balance (LLB + Group + Other - Expenses) */}
          <Card className="border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800">
            <CardHeader className="pb-2">
              ...
              <CardDescription className="text-slate-600 dark:text-slate-400">
                LLB Fees + Group Fees + Other Income - Expenses
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="p-4 bg-gradient-to-br from-green-50 to-emerald-50 dark:from-green-950/20 dark:to-emerald-950/20 rounded-xl border border-green-100 dark:border-green-900/30">
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-slate-600 dark:text-slate-400">LLB Fees</span>
                    <span className="font-semibold text-slate-900 dark:text-white">Rs {(stats?.todayLlbIncome || 0).toLocaleString()}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-slate-600 dark:text-slate-400">Group Fees</span>
                    <span className="font-semibold text-slate-900 dark:text-white">Rs {(stats?.todayGroupIncome || 0).toLocaleString()}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-slate-600 dark:text-slate-400">Other Income</span>
                    <span className="font-semibold text-slate-900 dark:text-white">Rs {(stats?.todayOtherIncome || 0).toLocaleString()}</span>
                  </div>
                  <div className="flex items-center justify-between border-t border-green-200 dark:border-green-800 pt-2">
                    <span className="text-sm font-medium text-slate-700 dark:text-slate-300">Total Income</span>
                    <span className="font-bold text-green-600 dark:text-green-400">Rs {(stats?.combinedTotalIncome || 0).toLocaleString()}</span>
                  </div>
                  <div className="flex items-center justify-between text-orange-600 dark:text-orange-400">
                    <span className="text-sm">(-) Expenses</span>
                    <span className="font-semibold">Rs {(stats?.todayExpenses || 0).toLocaleString()}</span>
                  </div>
                  <div className="flex items-center justify-between border-t-2 border-green-300 dark:border-green-700 pt-3 mt-2">
                    <span className="text-lg font-bold text-slate-900 dark:text-white">Balance</span>
                    <span className={`text-2xl font-heading font-bold ${(stats?.combinedBalance || 0) >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`} data-testid="stat-combined-balance">
                      Rs {(stats?.combinedBalance || 0).toLocaleString()}
                    </span>
                  </div>
                </div>
              </div>
          ...
          {/* Report 2: Group Only Balance (Group + Other - Expenses, NO LLB) */}
          <Card className="border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800">
            <CardHeader className="pb-2">
              <CardTitle className="font-heading text-slate-900 dark:text-white flex items-center gap-2">
              ...
              <CardDescription className="text-slate-600 dark:text-slate-400">
                Group Fees + Other Income - Expenses (LLB excluded)
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="p-4 bg-gradient-to-br from-teal-50 to-cyan-50 dark:from-teal-950/20 dark:to-cyan-950/20 rounded-xl border border-teal-100 dark:border-teal-900/30">
                <div className="space-y-3">
                  ...
                  <div className="flex items-center justify-between border-t border-teal-200 dark:border-teal-800 pt-2">
                    <span className="text-sm font-medium text-slate-700 dark:text-slate-300">Total Income</span>
                    <span className="font-bold text-teal-600 dark:text-teal-400">Rs {(stats?.groupTotalIncome || 0).toLocaleString()}</span>
                  </div>
                  <div className="flex items-center justify-between text-orange-600 dark:text-orange-400">
                    <span className="text-sm">(-) Expenses</span>
                    <span className="font-semibold">Rs {(stats?.todayExpenses || 0).toLocaleString()}</span>
                  </div>
                  <div className="flex items-center justify-between border-t-2 border-teal-300 dark:border-teal-700 pt-3 mt-2">
                    <span className="text-lg font-bold text-slate-900 dark:text-white">Balance</span>
                    <span className={`text-2xl font-heading font-bold ${(stats?.groupDailyBalance || 0) >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`} data-testid="stat-group-daily-balance">
