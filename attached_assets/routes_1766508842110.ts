...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get('/api/dashboard/admin', isAuthenticated, hasRole('admin'), async (req, res) => {
    try {
      ...
      // Calculate total income by division
      const totalLlbIncome = todayLlbFeeIncome;
      const totalGroupIncome = todayGroupFeeIncome + todayRecordedIncome; // Other income goes to Group
      
      // Expense deduction logic:
      // Rule 1: If Group has income, deduct ALL expenses from Group (even if negative)
      // Rule 2: Only deduct from LLB if Group income is ZERO and LLB has income
      // Rule 3: If both are zero, track expenses separately (not deducted from either)
      let llbBalance = totalLlbIncome;
      let groupBalance = totalGroupIncome;
      let unallocatedExpenses = 0;
      
      if (totalGroupIncome > 0) {
        // Group has income - all expenses come from Group
        groupBalance = totalGroupIncome - todayExpenses;
      } else if (totalLlbIncome > 0) {
        // No Group income but LLB has income - expenses come from LLB
        llbBalance = totalLlbIncome - todayExpenses;
      } else {
        // Neither has income - track expenses as unallocated
        unallocatedExpenses = todayExpenses;
      }
      ...
      res.json({
        ...
        dailyBalance: Math.round(llbBalance + groupBalance),
        ...
        todayLlbIncome: Math.round(todayLlbFeeIncome),
        todayGroupIncome: Math.round(totalGroupIncome),
        todayExpenses: Math.round(todayExpenses),
        unallocatedExpenses: Math.round(unallocatedExpenses),
      });
    } catch (error) {
  ...
  app.post('/api/fees/vouchers/bulk', isAuthenticated, hasRole('admin', 'accountant'), async (req, res) => {
    try {
      ...
      for (const student of matchingStudents) {
        const prefix = periodLabel.substring(0, 3).toUpperCase();
        const voucherNumber = `FEE-${prefix}-${year}-${Date.now()}-${Math.random().toString(36).substr(2, 6).toUpperCase()}`;
