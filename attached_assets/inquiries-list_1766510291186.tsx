...
export default function InquiriesList() {
  ...
  const form = useForm<Partial<InsertInquiry>>({
    resolver: zodResolver(insertInquirySchema.partial()),
    defaultValues: {},
  });

  const updateMutation = useMutation({
    mutationFn: async (data: Partial<InsertInquiry>) => {
      if (!selectedInquiry) throw new Error("No inquiry selected");
      return await apiRequest('PUT', `/api/inquiries/${selectedInquiry.id}`, data);
    },
    onSuccess: () => {
      toast({
        title: "Success",
  ...
  const handleEdit = (inquiry: Inquiry) => {
    ...
    form.reset({
      ...
      status: (inquiry.status ?? "pending") as "pending" | "contacted" | "admitted" | "rejected",
  ...
  const onSubmit = (data: Partial<InsertInquiry>) => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-heading font-bold" data-testid="heading-inquiries">Admission Inquiries</h1>
          <p className="text-muted-foreground">Manage prospective student inquiries</p>
        </div>
        <div className="flex gap-2">
          <Button 
            variant="outline" 
            onClick={() => setLocation('/admissions/inquiries/print')}
            data-testid="button-print-view"
          >
            <Printer className="h-4 w-4 mr-2" />
            Print View
          </Button>
          <Button onClick={() => window.location.href = '/admissions/inquiries/add'} data-testid="button-add-inquiry">
              ...
              <Table>
                ...
                <TableBody>
                  {inquiries.map((inquiry) => (
                    <TableRow key={inquiry.id} className="hover-elevate" data-testid={`row-inquiry-${inquiry.id}`}>
                      ...
                      <TableCell>
                        <Badge variant={getStatusVariant(inquiry.status || 'pending')} data-testid={`badge-status-${inquiry.id}`}>
                          {inquiry.status}
                        </Badge>
                      </TableCell>
      ...
      <Dialog open={editDialogOpen} onOpenChange={setEditDialogOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>Update inquiry information</DialogDescription>
          </DialogHeader>
          <Form {...form}>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently delete the inquiry for "{selectedInquiry?.name}". This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
